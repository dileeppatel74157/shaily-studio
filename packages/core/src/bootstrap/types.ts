...
export class BootstrapValidationException extends BootstrapException {
  constructor(message: string) {
    super(message);
  }
}

export class InvalidBootstrapStateException extends BootstrapException {
  constructor(action: string, currentState: BootstrapState) {
    super(`Cannot perform "${action}" while Bootstrapper is in "${currentState}" state.`);
  }
}
