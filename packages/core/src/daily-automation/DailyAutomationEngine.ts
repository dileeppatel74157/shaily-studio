...
class PipelineManagerImpl implements IPipelineManager {
  ...
  public async executePublishingRoutine(): Promise<Record<string, string>> {
    const pub = this.engine.getEngine<any>("PublishingEngine");
    if (pub && pub.execute) {
      try {
        const res = await pub.execute({
          ...
          target: {
            platform: "YOUTUBE",
            account: {
              accountId: "UC-mock-channel-123",
              displayName: "Shaily AI Studio Channel",
              platformUserId: "UC-mock-channel-123",
              provider: "YOUTUBE",
              status: "CONNECTED",
          ...
          videoPath: "video_render_draft.mp4",
          options: {
            metadataOverrides: {
              title: "Daily Auto-Generated Show",
              description: "This video was fully generated by Shaily Studio AI automation.",
              category: "Entertainment",
              tags: ["ai", "automation", "shaily"]
            }
          }
        });

        const ytJob = res.jobs?.find((j: any) => j.target.platform === "YOUTUBE" || j.target.platform === "youtube");
        if (ytJob && ytJob.platformVideoId) {
          return { "YouTube": ytJob.platformVideoId };
        }
      } catch (err) {}
    }
