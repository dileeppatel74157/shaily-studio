...
export class HostValidationException extends HostException {
  constructor(message: string) {
    super(message);
  }
}

export class InvalidHostStateException extends HostException {
  constructor(action: string, currentState: HostState) {
    super(`Cannot perform "${action}" while Host is in "${currentState}" state.`);
  }
}
