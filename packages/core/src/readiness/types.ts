export class ReadinessException extends Error {
...
export class ReadinessValidationException extends ReadinessException {
  constructor(message: string) {
    super(message);
  }
}

export class InvalidReadinessStateException extends ReadinessException {
  constructor(action: string, currentState: string) {
    super(`Cannot perform "${action}" while Readiness is in "${currentState}" state.`);
  }
}
